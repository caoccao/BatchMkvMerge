...
fn parses_complete_matroska_with_three_tracks() {
  ...
  assert_eq!(m.container.properties.title.as_deref(), Some("Full Synthetic Clip"),);
...
fn missing_file_returns_io_error() {
  let err = parse("definitely-does-not-exist-99999.mkv", ParseOptions::default()).unwrap_err();
