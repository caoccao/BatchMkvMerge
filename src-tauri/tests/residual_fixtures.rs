...
fn parses_flv_avc_aac_clip() {
    ...
    // Video tag with H.264 codec id
    blob.extend(build_flv_tag(0x09, &[(1 << 4) | 7, 0, 0, 0, 0]));
    // Audio tag with AAC (44.1k stereo 16-bit)
    let audio_byte = (10 << 4) | (3 << 2) | (1 << 1) | 1;
    blob.extend(build_flv_tag(0x08, &[audio_byte, 0]));
...
fn build_video_props(fourcc: &[u8; 4], width: u16, height: u16, fps: f64) -> Vec<u8> {
    ...
    let fps_q16 = ((fps.trunc() as u32) << 16)
        | (((fps.fract() * 65536.0).round() as u32) & 0xFFFF);
