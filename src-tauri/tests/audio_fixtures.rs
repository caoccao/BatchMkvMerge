...
#[test]
fn parses_tta() {
  let frame_bytes = 4096u32;
  let mut bytes = Vec::with_capacity(30 + frame_bytes as usize);
...
fn build_mp3_frame(bitrate_kbps: u32, sample_rate: u32) -> Vec<u8> {
  const BITRATE_TABLE_V1_LAYER3: [u32; 16] = [0, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320, 0];
  const SAMPLE_RATE_TABLE_V1: [u32; 4] = [44_100, 48_000, 32_000, 0];
  let bitrate_index = BITRATE_TABLE_V1_LAYER3.iter().position(|&b| b == bitrate_kbps).unwrap() as u8;
  let sr_index = SAMPLE_RATE_TABLE_V1.iter().position(|&s| s == sample_rate).unwrap() as u8;
