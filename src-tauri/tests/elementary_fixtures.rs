...
fn write_dirac_uint(w: &mut BitWriter, value: u32) {
  let mut count = 0u32;
  loop {
    ...
    if value >= base && value < base + capacity {
      ...
      for shift in (0..count).rev() {
        w.write_bit(false);
        w.write_bit(((remainder >> shift) & 1) != 0);
      }
      w.write_bit(true);
      return;
    }
...
fn build_dirac_sequence_header() -> Vec<u8> {
  let mut bytes = b"BBCD".to_vec();
  bytes.push(0x00);
  bytes.extend_from_slice(&[0u8; 8]);
  let mut w = BitWriter::new();
  write_dirac_uint(&mut w, 2);
  write_dirac_uint(&mut w, 2);
...
#[test]
fn parses_dirac() {
  let bytes = build_dirac_sequence_header();
...
fn parses_avc_baseline_1080p() {
  ...
  bytes.extend_from_slice(&[0x00, 0x00, 0x00, 0x01, 0x65, 0x80]);
