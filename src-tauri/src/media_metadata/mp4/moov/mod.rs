  ...
  pub fn finalise_container(&self, props: &mut ContainerProperties) {
    ...
    if let (Some(ts), Some(units)) = (self.timescale, self.duration_units) {
      if ts > 0 {
        ...
        props.duration = Some(crate::media_metadata::model::duration::DurationValue::from_ns(
          ns as u64,
        ));
      }
    }
...
fn handle_cmov(
  ...
  let uncompressed_size = u32::from_be_bytes([cmvd[0], cmvd[1], cmvd[2], cmvd[3]]) as usize;
