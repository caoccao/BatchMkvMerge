...
use crate::media_metadata::model::track_properties_video::{
    AlphaMode, Chromaticity, ColorMetadata, ColorRange, CropRect, Dimensions2D, DisplayUnit,
    FieldOrder, InterlaceFlag, MasterMetadata, ProjectionMetadata, ProjectionPose,
    ProjectionType, StereoMode, VideoTrackProperties,
};
...
impl VideoBuilder {
    pub fn build(self) -> VideoTrackProperties {
        ...
        let crop = if self.crop_left.is_some()
            || self.crop_top.is_some()
            || self.crop_right.is_some()
            || self.crop_bottom.is_some()
        {
            Some(CropRect {
...
mod tests {
    ...
    fn crop_emits_struct_when_any_field_set() {
        ...
        parse(&mut s, &h, &no_deadline(), &mut builder).unwrap();
        let v = builder.build();
        assert_eq!(v.crop, Some(CropRect { left: 8, top: 0, right: 0, bottom: 0 }));
    }
