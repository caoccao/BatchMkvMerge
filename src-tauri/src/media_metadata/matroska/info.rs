    ...
    ebml::walk_children(src, parent, "matroska::info", deadline, |src, child| {
        match child.id {
            ...
            ids::WRITING_APP => {
                ...
                out.container.properties.writing_app =
                    Some(parsed.into_display(&s));
                Ok(ChildAction::Consumed)
            }
            ids::SEGMENT_UID => {
            ...
            ids::DATE_UTC => {
                ...
                out.container.properties.date_utc =
                    Some(format_matroska_date(ns_since_matroska_epoch));
                Ok(ChildAction::Consumed)
            }
...
mod tests {
    ...
    fn date_utc_formats_as_iso_8601() {
        ...
        let payload = encode_element(
            ids::DATE_UTC,
            2,
            &one_year_ns.to_be_bytes(),
        );
        let (_b, header, mut s) = build_info(payload);
        let mut out = MediaMetadata::new("clip.mkv", 0);
        parse(&mut s, &header, &no_deadline(), &mut out).unwrap();
    ...
    fn matroska_epoch_unix_timestamp_known() {
        ...
        assert_eq!(
            format_unix_seconds_utc(MATROSKA_EPOCH_UNIX),
