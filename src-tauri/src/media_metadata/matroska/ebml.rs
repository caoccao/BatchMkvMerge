...
pub fn read_uint(src: &mut FileSource, header: &ElementHeader) -> Result<u64, ParseError> {
    let size = header.size.ok_or_else(|| ParseError::Malformed {
        ...
        offset: header.start,
        reason: format!(
            "uint element {:#x} has unknown size",
            header.id
        ),
    })?;
    if size > 8 {
        return Err(ParseError::Malformed {
            ...
            offset: header.start,
            reason: format!(
                "uint element {:#x} too large ({} bytes)",
                header.id, size
            ),
        });
    }
...
pub fn read_int(src: &mut FileSource, header: &ElementHeader) -> Result<i64, ParseError> {
    let size = header.size.ok_or_else(|| ParseError::Malformed {
        ...
        offset: header.start,
        reason: format!(
            "int element {:#x} has unknown size",
            header.id
        ),
    })?;
    if size > 8 {
...
pub fn read_float(src: &mut FileSource, header: &ElementHeader) -> Result<f64, ParseError> {
    let size = header.size.ok_or_else(|| ParseError::Malformed {
        ...
        offset: header.start,
        reason: format!(
            "float element {:#x} has unknown size",
            header.id
        ),
    })?;
    match size {
...
pub fn read_string(
    ...
    String::from_utf8(slice.to_vec()).map_err(|e| ParseError::Malformed {
        ...
        offset: header.start,
        reason: format!(
            "string element {:#x} is not valid UTF-8: {}",
            header.id, e
        ),
    })
}
...
pub fn read_binary(
    ...
    let size = header.size.ok_or_else(|| ParseError::Malformed {
        ...
        offset: header.start,
        reason: format!(
            "binary element {:#x} has unknown size",
            header.id
        ),
    })?;
    if size > cap {
...
mod tests {
    ...
    fn read_binary_enforces_cap() {
        ...
        assert!(matches!(
            err,
            ParseError::OversizedElement { format: "matroska", .. }
        ));
    }
    ...
    fn walk_children_errors_when_child_extends_past_parent() {
        ...
        let h = read_element_header(&mut s).unwrap();
        let deadline = no_deadline();
        let err = walk_children(&mut s, &h, "test", &deadline, |_, _| {
            Ok(ChildAction::Skip)
        })
        .unwrap_err();
        assert!(matches!(err, ParseError::Malformed { .. }));
