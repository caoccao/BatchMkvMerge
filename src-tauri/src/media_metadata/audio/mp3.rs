...
pub fn decode_mp3_header(buf: &[u8]) -> Option<Mp3Header> {
    ...
    let framesize = if layer == 3 {
        (if version == 1 { 144000 } else { 72000 }) * bitrate as usize / sampling_frequency as usize
            + padding as usize
    } else if layer == 2 {
...
impl Reader for Mp3Reader {
    ...
    fn read_headers(
        ...
        let (_offset, frame) = find_consecutive_mp3_headers(bytes, MIN_CONFIRM_FRAMES)
...
mod tests {
    ...
    fn consecutive_num_one_is_first_header() {
        ...
        assert_eq!(
            find_consecutive_mp3_headers(&bytes, 1).map(|(o, _)| o),
            Some(0)
        );
    }
