...
impl Header {
    ...
    fn packet_length_ns(&self) -> Option<i128> {
        if self.has_core {
            ...
            if self.reference_clock_code < 3 {
                Some(
                    self.substream_frame_duration as i128 * 1_000_000_000
                        / PERIODS[self.reference_clock_code as usize],
                )
            } else {
                None
            }
        ...
        if self.static_fields_present {
            ...
            if self.mix_metadata_enabled {
                ...
                for mix_idx in 0..self.num_mixing_configurations.min(5) as usize {
                    self.num_mixing_channels[mix_idx] =
                        count_channels_for_mask(br.read_bits(speaker_mask_num_bits).ok()? as u32);
                }
            }
        }
    ...
    fn parse_xll_parameters(&mut self, br: &mut BitReader<'_>, asset_idx: usize) -> Option<()> {
        self.substream_assets[asset_idx].xll_size =
            br.read_bits(self.substream_size_bits).ok()? as usize + 1;
        ...
        if self.substream_assets[asset_idx].xll_sync_present {
            ...
            self.substream_assets[asset_idx].xll_sync_offset =
                br.read_bits(self.substream_size_bits).ok()? as usize;
        } else {
...
impl Reader for DtsReader {
    ...
    fn read_headers(
        ...
        let (_offset, header) =
            find_header(&transformed, false).ok_or(ParseError::Unrecognised)?;
