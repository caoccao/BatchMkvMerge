  ...
  fn read_headers(
    ...
    if let Some(ac3) = ac3_frame {
      ...
      for track in &mut out.tracks {
        if matches!(track.id, 0 | 1) {
          track.properties.common.multiplexed_with = vec![0, 1];
        }
      }
    }
    Ok(())
...
mod tests {
  ...
  fn read_headers_exposes_ac3_substream() {
    ...
    assert_eq!(out.tracks[0].properties.common.multiplexed_with, vec![0, 1]);
    assert_eq!(out.tracks[1].properties.common.multiplexed_with, vec![0, 1]);
  }
