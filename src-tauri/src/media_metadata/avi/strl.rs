...
fn parse_bitmapinfoheader(bytes: &[u8], offset: u64) -> Result<StreamFormat, ParseError> {
  if bytes.len() < 40 {
    return Err(ParseError::Malformed {
      format: "avi",
      offset,
      reason: format!("BITMAPINFOHEADER payload {} bytes too small", bytes.len()),
    });
  }
...
fn parse_waveformatex(bytes: &[u8], offset: u64) -> Result<StreamFormat, ParseError> {
  ...
  let extra = if bytes.len() > extra_offset + 2 {
    let cb_size = u16::from_le_bytes([bytes[extra_offset], bytes[extra_offset + 1]]) as usize;
...
pub fn parse_strl(
  ...
  riff::walk_list_children(src, parent, "avi::strl", deadline, |src, child| match &child.kind {
    b"strh" => {
  ...
  if builder.format.is_none() {
    if let (Some(off), Some(size), Some(h)) = (deferred_strf_offset, deferred_strf_size, builder.header.clone()) {
      src.seek_to(off)?;
...
#[cfg(test)]
pub(crate) fn build_bitmapinfoheader(width: i32, height: i32, bit_count: u16, compression: &[u8; 4]) -> Vec<u8> {
...
mod tests {
  ...
  fn strl_with_video_strh_and_strf() {
    ...
    let strf = encode_chunk(b"strf", &build_bitmapinfoheader(1920, 1080, 24, b"H264"));
  ...
  fn strl_extracts_strn_name() {
    let strh = encode_chunk(b"strh", &build_strh_payload(b"vids", b"H264", 1, 30, 0, 0));
  ...
  fn waveformatex_with_extra_bytes_round_trips() {
    ...
    let strh = encode_chunk(b"strh", &build_strh_payload(b"auds", b"\0\0\0\0", 1, 48000, 0, 0));
  ...
  fn bitmapinfoheader_uncompressed_predicate() {
    ...
    let dib = BitmapInfoHeader {
      compression: *b"DIB ",
      ..bmih.clone()
    };
    assert!(dib.is_uncompressed());

    let h264 = BitmapInfoHeader {
      compression: *b"H264",
      ..bmih
    };
