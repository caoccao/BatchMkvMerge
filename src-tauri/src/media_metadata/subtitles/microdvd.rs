...
impl Reader for MicroDvdReader {
    ...
    fn read_headers(
        ...
        out.tracks.push(Track {
            ...
            codec: CodecInfo {
                id: "S_TEXT/MICRODVD".to_string(),
                name: Some("MicroDVD".to_string()),
                codec_private: None,
            },
            properties: TrackProperties {
                ...
                subtitle: Some(SubtitleTrackProperties {
                    ...
                    variant: Some("MicroDVD".to_string()),
                    teletext_page: None,
                }),
...
mod tests {
    ...
    #[test]
    fn read_headers_emits_microdvd_track() {
        use crate::media_metadata::deadline::Deadline;
        let blob = b"{1}{125}Hello\n";
        let mut s = FileSource::from_reader_for_test(Cursor::new(blob.to_vec()));
        let mut out = MediaMetadata::new("clip.sub", 0);
        MicroDvdReader.read_headers(&mut s, &Deadline::new(60_000), &mut out).unwrap();
        assert_eq!(out.container.format, ContainerFormat::MicroDvd);
        assert_eq!(out.tracks[0].codec.id, "S_TEXT/MICRODVD");
    }
