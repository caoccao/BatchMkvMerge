...
use super::identify;
use super::packet::{self, detect_packet_size_aligned, PACKET_SIZE_BD_M2TS};
use super::pat;
use super::pmt;
...
impl Reader for MpegTsReader {
    ...
    fn read_headers(
        ...
        let (packet_size, start) =
            detect_packet_size_aligned(&probe[..probe_len]).ok_or(ParseError::Unrecognised)?;
        ...
        loop {
            ...
            if !pmt_pids.is_empty()
                && seen_pmt_pids.len() == pmt_pids.len()
                && (!sdt_map.is_empty() || packet_count > 4096)
            {
                break;
            }
        }
