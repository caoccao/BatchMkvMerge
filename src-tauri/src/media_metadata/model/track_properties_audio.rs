...
mod tests {
  ...
  fn channel_layout_kind_round_trip() {
    for kind in [
      ...
      let back: ChannelLayoutKind = serde_json::from_str(&serde_json::to_string(&kind).unwrap()).unwrap();
      assert_eq!(back, kind);
    }
  }
  ...
  fn emphasis_round_trip() {
    for e in [
      ...
      let back: AudioEmphasis = serde_json::from_str(&serde_json::to_string(&e).unwrap()).unwrap();
      assert_eq!(back, e);
    }
  }
