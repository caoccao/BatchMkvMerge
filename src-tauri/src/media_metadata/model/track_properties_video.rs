...
mod tests {
  ...
  fn color_metadata_round_trip_with_master() {
    let color = ColorMetadata {
      ...
      master: Some(MasterMetadata {
        ...
        white_point: Some(Chromaticity { x: 0.3127, y: 0.329 }),
  ...
  fn stereo_mode_round_trip() {
    for mode in [
      ...
      let back: StereoMode = serde_json::from_str(&serde_json::to_string(&mode).unwrap()).unwrap();
      assert_eq!(back, mode);
    }
  }
  ...
  fn field_order_round_trip() {
    for f in [
      ...
      let back: FieldOrder = serde_json::from_str(&serde_json::to_string(&f).unwrap()).unwrap();
      assert_eq!(back, f);
    }
  }
