...
- `PARSER-397` - The bounded SRT probe still reads too many bytes for the first cue-index line. Mkvtoolnix calls `mm_text_io_c::getline(10)` after seeking past any BOM, so a non-BOM line longer than 10 decoded characters is split and the following `getline(100)` sees the remainder of that same line. Rust's `read_probe_line` uses `max_chars + 3` for UTF-8-like input (and extra units for UTF-16), so an 11-13 digit first line can be read as one complete numeric index and the real timestamp line can make the file claim as SRT even though mkvtoolnix rejects it.
- `PARSER-398` - The empty `.srt` extension fallback applies the `<= 1` threshold to raw file size instead of `size - BOM length`. Mkvtoolnix subtracts `mm_text_io_c::get_byte_order_length()` before accepting empty/tiny SRT files by extension, so BOM-only UTF-8 or UTF-16 `.srt` files are accepted upstream but rejected locally.
