...
Registration descriptors on private PES streams mirror mkvtoolnix's `codec_c` lookup (PARSER-384). HDMV registration descriptors require the full 8-byte descriptor body plus the `0xFF` stuffing byte before reading `stream_coding_type`; supported Blu-ray coding types map through the upstream set, while MPEG-1 video (`0x01`), MVC-dependent AVC (`0x20`), and interactive graphics (`0x91`) remain unsupported and are left unknown. Non-HDMV registration FourCCs are resolved through the codec table used by `codec_c::look_up`, so registered identifiers such as AV1/AVC/HEVC, MPEG-1/2, MPEG-4 Part 2, AC-3, DTS, AAC/MP4A, PCM, RealAudio, Kate, TX3G, and USF are accepted when upstream would accept them; local-only aliases such as `BSSD` and `DTS1`/`DTS2`/`DTS3` are not promoted.

PMT program descriptors are parsed only as part of validating and stepping over the program-info loop. They are not fed into per-stream row construction: ISO-639 language descriptors, service descriptors, and codec-disambiguating descriptors must appear in the elementary-stream descriptor loop to affect tracks. Descriptor languages are surfaced only when the decoded value has a valid ISO-639 code, so invalid ISO-639, teletext, or DVB-subtitle descriptor bytes leave the language absent instead of being repaired to `und` (PARSER-364). DVB subtitling descriptors create a single `S_DVBSUB` row from the first 8-byte descriptor entry, matching `parse_subtitling_pmt_descriptor`; remaining entries in the same descriptor are ignored (PARSER-365). Teletext page nibbles are used directly without BCD validation, mirroring mkvtoolnix's `tens * 10 + units` calculation for malformed-but-visible pages (PARSER-366). Service provider/name metadata comes from SDT service descriptors, mirroring mkvtoolnix's separation between PMT stream metadata and DVB service metadata.
...
The scan is fixed and bounded, so metadata that appears very late can be missed. Upstream also performs timestamp continuity handling, CLPI-assisted source packet trimming, packet muxing, and a larger descriptor universe. Rust records the best available program/track metadata and avoids long-running payload walks. PAT/PMT now reject inactive and multi-section tables, per-PID PES accumulation strips every PES header and keeps up to the 5 MiB probe horizon, AVC/HEVC probes use complete mkvtoolnix-style elementary-stream gates, registration descriptors use the mkvtoolnix codec lookup surface, Teletext/TextST subtitle rows keep `textSubtitles: false` like mkvtoolnix's TS identification path, PMT program descriptors are not rewritten as track metadata, descriptor languages/subtitling/teletext page handling match mkvtoolnix's PMT descriptor behavior, and in-stream sync loss is recovered with a bounded resync scan.
