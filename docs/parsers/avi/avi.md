...
- `PARSER-348`: GAB2 subtitle extraction is still bounded in ways avilib is not. Local subtitle discovery scans only 4096 `movi` child chunk headers, reads only the first 1 MiB of a matching `NNtx` text chunk, and classifies only the first 1 MiB of the embedded GAB2 subtitle block. mkvtoolnix uses avilib's text-track index, reads the full first text chunk for each text track, and allocates the declared subtitle block length. Late text chunks, large SSA/ASS payloads, and attachments after the first MiB can be missed or truncated locally.
- `PARSER-349`: MPEG-4 Part 2 display-dimension recovery can miss the first frame that mkvtoolnix reads. Local VOL PAR discovery linearly scans at most 4096 `movi` child chunks and reads a 256 KiB prefix of the first matching video chunk. `avi_reader_c::extended_identify_mpeg4_l2` asks avilib for video frame 0 from the AVI index and reads that whole frame. Interleaved files whose first video frame appears after many audio/subtitle chunks, or whose VOL header lies beyond the 256 KiB prefix, lose display aspect ratio locally.
