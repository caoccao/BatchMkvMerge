...
- **Kate** (`codecs/kate.rs`) — keeps reading header packets while the high bit of the first byte is set (`r_ogm.cpp:1707-1710`) and Xiph-laces all of them into codec private (`r_ogm.cpp:1678` → `lace_memory_xiph`). Header collection stops at the first high-bit-clear packet rather than a fixed packet-count ceiling. Kate and OGM text subtitle tracks report `text_subtitles=true` and `encoding="UTF-8"`, matching mkvtoolnix's identify path (PARSER-381).
...
The page loop mirrors libogg sync recovery for damaged capture patterns. When a page header does not start with `OggS`, the reader scans forward in bounded overlapping windows for the next `OggS` capture pattern and resumes there, so recoverable junk before later BOS, comment, or header pages does not hide streams and tags. Header reading requires at least one beginning-of-stream page and requires the scan to reach the first non-BOS page before finalisation; missing-BOS and BOS-only inputs fail like mkvtoolnix's `bos_pages_read` guard instead of producing a recognised empty container (PARSER-390). After that boundary, reading continues until EOF/truncation, every active codec reports its headers complete, or the configured deadline expires; once all active streams have `headers_read`, the reader exits immediately and does not accept non-conforming late BOS pages (PARSER-380). Packets that span pages are reassembled until their Ogg lacing terminator, so large Vorbis/Opus/Theora/VP8 comment packets and variable codec header packets are handled like libogg rather than being dropped at a 16 MiB local limit. VorbisComment `LANGUAGE` and Kate language hints are only surfaced when they resolve to a known ISO-639 code; invalid hints are omitted instead of being repaired to `und` (PARSER-367).
...
The Rust parser uses bounded per-page payload reads and does not perform full granule-position timing, packet muxing, or every upstream comment edge case. VP8-in-Ogg is recognised, both FLAC-in-Ogg wrappers plus variable-length Kate headers are assembled until their codec-level terminators, damaged capture patterns are resynchronised to later `OggS` pages, files without a BOS page and BOS-only files are rejected before finalisation, late BOS pages after the active headers complete are ignored like upstream, and invalid language hints are omitted. The parser reports the header metadata needed for listing streams and leaves timing reconstruction to mkvmerge.
