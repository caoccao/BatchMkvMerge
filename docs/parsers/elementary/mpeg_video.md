...
Upstream uses the full `M2VParser` state machine while reading actual frames. Rust mirrors the probe predicates and validates the first frame-like structure, but it still does not retain parser state for muxing-grade packet validation. The metadata it reports follows mkvtoolnix's identify-time sequence-header interpretation, while full frame delivery remains outside this parser.
