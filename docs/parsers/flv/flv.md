...
Rust extracts selected AMF fields and does not perform timestamp/min-offset work or packet muxing. AVC/HEVC now mirror upstream's SPS-timing-then-25-fps default-duration fallback and keep sequence-header tracks even when config parsing cannot recover dimensions or the config payload is empty. Unsupported Screen video codecs are dropped like upstream, encrypted payloads are skipped rather than parsed, MP3 format-14 uses the normal FLV rate bits, FLV audio bit depth remains unset like mkvtoolnix identification, and AAC discovery follows upstream's packet-type gate plus flag-derived fallback behavior.
