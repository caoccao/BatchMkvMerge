...
- `PARSER-363` - Invalid packed `mdhd` language values are rewritten to `und`. mkvtoolnix decodes the three 5-bit letters and passes the result to `language_c::parse`; invalid results remain invalid and `get_iso639_alpha_3_code()` identifies no language. The Rust `decode_packed_language` maps all-zero or non-lowercase outputs to `und`, inventing an explicit language value.
