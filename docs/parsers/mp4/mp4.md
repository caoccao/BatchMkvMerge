...
The parser scans top-level boxes, handles normal and zlib-compressed `moov` boxes, parses `ftyp`, `mvhd`, `trak`, `tkhd`, `mdia`, `mdhd`, `hdlr`, `stbl`, `stsd`, `stts`, `stsc`, `edts/elst`, `mvex/trex`, `moof/traf/tfhd/trun`, and `udta/meta/ilst`. Track-level `trak/udta/name` atoms are stripped and the first non-empty value becomes `CommonTrackProperties.track_name`, matching `handle_trak_udta_atom` added upstream in June 2026. The `mdia/hdlr` component name is retained only as parser state and is no longer misreported as the user-facing track name (PARSER-403). Compressed QuickTime `cmov` handling reads the declared `cmvd` payload and inflates up to the movie size declared inside the compressed header, rather than using the old 64 MiB local cap. Codec-specific parsers cover AVC, HEVC, AV1, AAC, ALAC, Opus, FLAC, QuickTime PCM, color, pixel aspect ratio, and Dolby Vision block-addition records.
...
- **AAC (`esds`)** — the DecoderSpecificInfo bytes are parsed through the shared AAC AudioSpecificConfig decoder used by the elementary readers. Parsed AAC channels, sample rate, and SBR output rate override the sample-entry placeholders; if an AAC `esds` omits DecoderSpecificInfo or carries a nonzero-but-too-short value, a default AudioSpecificConfig is synthesised from the sample-entry channels and sample rate, matching `parse_aac_esds_decoder_config`. A present zero-length DecoderSpecificInfo descriptor is rejected as malformed during `esds` parsing, matching `parse_esds_atom`'s `header_parsing_x` path (PARSER-371).
...
HEVC verification mirrors mkvtoolnix's Annex B fallback. Before rejecting an `hvc1` / `hev1` track without `hvcC`, the verifier checks whether the indexed first sample starts with `00 00 00 01`, then reads up to 1 MiB of first-sample bytes and feeds the existing elementary HEVC parser. If VPS/SPS/PPS headers plus a parsed and flushed first access unit are recovered, it builds an `hvcC` configuration record, stores it as codec private, and applies the normal 23-byte HEVC config gate (PARSER-313, PARSER-351).

First-sample verification reconstructs only the sample extents covered by `stsc` + `stco`/`co64` + `stsz`; compact `stz2` boxes are skipped because mkvtoolnix's `handle_stbl_atom` has no `stz2` branch. Missing `stsc` coverage produces no synthetic one-sample chunks, and zero or missing sample sizes remain zero-sized index entries (PARSER-299). `stsz` fixed sizes and per-sample entries at or above 100 MiB are zeroed before index construction, matching `handle_stsz_atom`'s corrupt-size guard. `stsc`, `stsz`, `stco`, and `co64` validate that their atom payloads contain the full declared entry count before retaining the bounded prefix needed for the header-only first-sample index, so truncated declared tables fail like mkvtoolnix's exact `read_uint*_be` loops instead of being repaired into partial indexes (PARSER-372). The first-sample index is retained up to the 1 MiB HEVC Annex B salvage window, so fragmented runs of small samples remain available when `verify.rs` requests 128 KiB through 1 MiB. `read_first_bytes` then mirrors `qtmp4_demuxer_c::read_first_bytes`: it reads `min(remaining, sample.size)` from each indexed sample and returns no buffer unless the full requested window (64, 10000, 16384, or up to 1 MiB for HEVC Annex B salvage) is collected with exact media reads (PARSER-298, PARSER-341). Truncated `mdat` data, short indexed windows, and zero-sized samples therefore cannot satisfy MP3/AC-3/DTS parameter recovery or AVC/HEVC bitstream salvage.
...
Recognised `esds` objectTypeIndication values are applied before sample-entry FOURCC fallback for every sample entry, matching `qtmp4_demuxer_c::determine_codec`: AAC (`0x40`, `0x66`-`0x68`), MP3 (`0x69`), MP2 (`0x6B`), DTS (`0xA9`), Vorbis (`0xDD`), MPEG-1/2 video (`0x60`-`0x65`, `0x6A`), MPEG-4 Visual (`0x20`), and VobSub (`0xE0`). All MPEG-1/2 Visual object types map to the single upstream `V_MPEG12` / `MPEG-1/2` codec family; mkvtoolnix chooses the MPEG-1 versus MPEG-2 packetizer version later from the object type, which is outside the header-only metadata surface (PARSER-385). Object types outside that set, including AC-3/E-AC-3 private values, HEVC, AVC, and JPEG IDs not recognised by mkvtoolnix's table, remain unsupported and are dropped or handled by the sample-entry FOURCC fallback just as upstream would (PARSER-315, PARSER-389).

iTunes `ilst` handling follows mkvtoolnix's identification surface: only `©nam` (container title), `©too` (muxing application), `©cmt` (global comment tag), and `covr` cover-art payloads are reported. Text `data` payloads for the supported fields are read under the shared element-size budget, decoded lossily as UTF-8, and stripped; unsupported normal atoms, dates, and freeform reverse-DNS fields are skipped. `----:iTunSMPB` is consumed as non-identification encoder-delay state upstream and is not exposed as a tag locally (PARSER-342).
...
Upstream has complete sample-table muxing, interleaving, and a wider QuickTime metadata surface, and reads chapter-track sample payloads to recover per-chapter titles and timecodes. Rust implements enough sample-table handling for first-sample verification and chapter counting but not packet output or chapter-name extraction. Rare atoms and codec branches are intentionally narrower; unknown private data is preserved where useful rather than interpreted unsafely. The Vorbis codec private kept on the model is the raw esds decoder configuration (informational); the muxing-time re-lacing into Matroska Vorbis private data is a packetizer concern out of scope for identification. The `hvcC` parser reads `chromaFormat` from byte 16, `bitDepthLumaMinus8` from byte 17 and `bitDepthChromaMinus8` from byte 18 (the avgFrameRate bytes 19-20 are ignored), matching `../mkvtoolnix/src/common/hevc/hevcc.cpp`. Packed `mdhd` language values are only surfaced when the decoded three-letter value is valid; all-zero/non-letter values remain absent instead of being repaired to `und` (PARSER-363). HEVC Annex B salvage now uses the elementary parser's mkvtoolnix-style first-access-unit flush gate; track-level `udta/name`, subtitle verification, `esds` object-type mapping including MPEG-1/2 Visual as `V_MPEG12` with mkvtoolnix precedence over sample-entry FOURCCs, compressed `cmov`, sample-table count validation, `stz2` skipping, 100 MiB `stsz` sample-size zeroing, `ilst` identification metadata, and codec-private preservation follow mkvtoolnix's identification gates.
