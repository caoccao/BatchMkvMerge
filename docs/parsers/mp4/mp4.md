...
Codec-private data is normalised to match the byte layout mkvtoolnix hands its packetizers. AVC/HEVC/AV1 configuration boxes, opaque sample-entry private tails, FLAC `dfLa`, and Dolby Vision block-addition boxes preserve the full declared header payload under the shared parser element-size budget rather than applying codec-local 4 KiB or 64 KiB caps. Descriptor bodies inside `esds` are validated exactly before their lengths or payloads are recorded, so malformed DecoderSpecificInfo descriptors cannot satisfy later codec verification gates:
