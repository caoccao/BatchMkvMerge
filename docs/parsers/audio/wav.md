...
The byte total now accumulates all data chunks like upstream, so duration is correct for multi-`data`-chunk files, the >4 GiB data-length repair matches `scan_chunks_wave`, and probe-time wrapper detection uses mkvtoolnix's minimum header length. Unsupported format tags are reported through the structured model rather than matching mkvmerge's exact text output.
